...
$(function(){
	...
	$("#addBtn").click(function(){
		url = '../../savePersonnelOfficialInfoOfficia.json';
	...
	//在职员工页面确认按钮绑定单击事件
	$("#submitOfficialBtn").click(savePersonnelOfficialInfo)
...
function userJurisdiction(){
	$.ajax({
		url:"../../userJurisdictionOfficia.json",
...
function loadDataToSelect(){
	//查询下拉列表需要加载的数据
	$.ajax({
		url:"../../loadDataToSelectOfficia.json",
		dataType:"json",
		success:function(data){
			//加载查询功能区的部门信息
			$("#wpId").combobox({
...
function searchPersonalData(){
	...
	$("#tt").datagrid({
		url:"../../searchPersonnelTestDataOfficia.json",
		columns:[[    
	        ...
	        {field:'startWorkTime',title:'到岗日期',width:'6%',align:'center',sortable:true},
	        {field:'workAge',title:'工龄',width:'6%',align:'center',sortable:true},
	        ...
	        {field:'isOfficial',title:'转正/试用',width:'6%',align:'center'},
	        {field:'turnOfficialTime',title:'转正日期',width:'6%',align:'center'},
 ...
 * 提交员工信息
 */
function savePersonnelOfficialInfo(){
	var personnelTestObj =  $("#personnel_official_form").serializeObject();
...
function optionToConfirm(){
	...
	//根据员工编号查询信息
	$.ajax({
		url:"../../searchPersonnelByUserIdOfficia.json",
		dataType:'json',
		async:false,
		...
		url = "../../saveModiftyOfficialPersonnelOfficia.json";
		
	}else if(modForm.cho == 1){//显示离职员工修改界面
...
function saveModifyLeavePersonnel(){
	$.ajax({
		url:'../../saveModifyLeavePersonnelOfficia.json',
		type:'post',
		dataType:'json',
...
function removePersonnelByUerId(){
	$.messager.confirm('确认', '您确认将该员工调整为离职员工吗吗？此操作不可返回请酌情考虑', function(r) {
		if (r) {
			...
			if (info.length > 0) {
				$.ajax({
					url:"../../removePersonnelByUerIdOfficia.json",
					dataType:"json",
					type:"post",
...
function exportSome(){
	...
	$.ajax({
		...
		success:function(data){
			if(data.result == '1'){
				$("#exportSome").attr('href','../../startExportSomeOfficia.json')
			}
