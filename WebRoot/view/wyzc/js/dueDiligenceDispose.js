...
$(function(){
	...
	//refreshWebPage()
	window.setInterval(refreshWebPage,6000);
	...
	$("#cancelDueBtn").click(function(){
		...
		/*$.ajax({
			url:"../../addRejectRemark.json",
			data:{userPs:$("#userPs").textbox('getText'),userId:userId,userPost:userPost},
...
function refreshWebPage(){
	...
	if(strUrl.split('/')[7] == 'dueDiligenceDispose.html'){
		showDueDiligenceDisposeData()
	}	
}
...
function showDueDiligenceDisposeData(){
	...
	$("#tt").datagrid({
		url:"../../showDueDiligenceDisposeData.json",
		columns:[[    
	       ...
	       /* {field:'userPostFile',title:'附件',width:'5%',align:'center',
	        	formatter:function(value,row,index){
	        		return "<a href='../../downFile.json?userChooseHouse="+row.userChooseHouse+"' style='color:blue'>点击下载</a>"
	        	}
	        }*/
	    ]]	   
	});
...
function particularsHouse(chooseHouse){
	$.ajax({
		url:"../../searchHouseUrlByChooseHouse.json",
