...
function showDueDiligenceDisposeData(){
	...
	$("#tt").datagrid({
		url:"../../showPreliminaryDueDiligenceDisposeData.json",
		columns:[[    
	        ...
	        {field:'userPostFile',title:'附件',width:'5%',align:'center',
	        	formatter:function(value,row,index){
	        		return "<a href='../../preliminaryDownFile.json?userId="+row.userId+"' style='color:blue'>点击下载</a>"
	        	}
	        }
	    ]]	   
	});
...
function amendUserPost(userPost,userPs){
	$.ajax({
		url:"../../amendPreliminaryUserPost.json",
...
function particularsHouse(chooseHouse){
	$.ajax({
		url:"../../searchPreliminaryHouseUrlByChooseHouse.json",
...
function refreshWebPage(){
	...
	if(strUrl.split('/')[7] == 'preliminaryOrder.html'){
		searchAllOrderr();
	}	
}