 ...
 * 分页查询
 */
function FeedbacksearchData(){
	$("#tt").datagrid(Common.createDatagridOptionsParams("","",135,null))
	$("#tt").datagrid({
		title:"数据列表",
		url:"../../searchProjectData.json?strStatus=001,004",
		columns:[[    
	        ...
	        {field:'modifyTime',title:'计划完成时间',width:'10%',align:'center'}, 
	        {field:'name',title:'操作',width:'11%',align:'center', 
	        	formatter:function(value,row,index){
	        			return '<a style="color:blue"  href="#" onclick="allocation('+row.issueId+','+index+')">'+"反馈"+'</a>';
	        	}
	        }     
	    ]]	   
	});
							...
							success:function(data){
								if(data.result == 0){
									...
									$('#commentTab').datagrid({    
									    ...
									    columns:[[    
									        {field:'logId',title:'缺陷序号',width:'17%'},    
