        ...
        formatData: function (data) {
            ...
            if (data.rows.length > 0) {
                ...
                $.each(data.rows, function (index, item) {
                    strs.push('<li rowno="' + index + '" style="position:relative">' + $this.options.render(item, index) + '</li>');
                })
                $this.$elem.find('.imggrid-list ul').html(strs.join(''));
                $this.$elem.find('.imggrid-list ul li').css('width', $this.options.img.width).css('height', "150px");
